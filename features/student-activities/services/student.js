...
export const studentService = {
  ...
  getStudentActivities: async (studentId, params = {}) => {
    try {
      ...
      console.error('Get student activities error:', error.response?.data || error.message);
    }
  },

  submitFeedback: async (id, data) => {
    try {
      const response = await axios.patch(`/api/daily-activities/${id}`, {
        status: data.status,
  ...
  deleteStudent: async (studentId) => {
    try {
      const response = await axios.delete(`${STUDENTS_API_URL}/${studentId}`);
      return response.data;
    } catch (error) {
