...
const applicationSchema = z
  ...
  .refine((data) => data.companyEngineerCount <= data.companyEmployeeCount, {
    message: 'Mühendis sayısı toplam çalışan sayısından fazla olamaz',
  ...
  .superRefine((data, ctx) => {
    ...
    if (new Date(data.internshipStartDate) < new Date(period.internshipStartDate)) {
      console.log('Başlangıç tarihi hatası');
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Staj başlangıç tarihi dönem başlangıcından önce olamaz',
        path: ['internshipStartDate'],
      });
    }

    // Seçilen bitiş tarihi kontrol
    if (new Date(data.internshipEndDate) > new Date(period.internshipEndDate)) {
